(function () {
    angular.module("AppModule")
            .controller("itemWiseController", function ($scope, $filter, $uibModal, $uibModalStack, PrintPane, printService, $rootScope, Factory, $location, Notification) {
                $scope.model = {};
                $scope.model.data = {};
...
//                var date = new Date();
                ...
                var getDetail = "/api/sv/reports/item-wise";
                ...
                $scope.ui.getDetail = function () {
                    ...
                    detail.fromTime = $filter('date')($scope.ui.component.fromTime, "HH:mm:ss");
                    detail.toTime = $filter('date')($scope.ui.component.toTime, "HH:mm:ss");
                    detail.typeItem = null;
                    detail.typeGift = null;
                    detail.isType = false;
                    ...
                    if (detail.item) {
                        detail.typeItem = 'Item';
                    }
                    if (detail.gift) {
                        detail.typeGift = 'Gift';
                    }
                    if (detail.gift || detail.item) {
                        detail.isType = true;
                    }
                    if (detail.barcode === "") {
                        detail.barcode = null;
                    ...
                    if (detail.terminal) {
                        detail.terminal = parseInt(detail.terminal);
                    }
                        ...
                        Factory.save(getDetail, JSON.stringify(detail),
                                function (data) {
                                    if (data) {
                                        ...
                                        console.log(data);
                                        $scope.ui.detailQty = 0;
                                        $scope.ui.detailDiscount = 0;
                                        $scope.ui.detailValue = 0;
                                        angular.forEach(data, function (data) {
                                            $scope.ui.detailQty += data[6];
                                            $scope.ui.detailDiscount += data[8];
                                            $scope.ui.detailValue += data[9];
                                        });
                                         $scope.ui.gifShow = false;
                ...
                $scope.ui.showParameeter = function () {
                    $scope.showParameeter = !$scope.showParameeter;
                };
